...
# 2. 交互模式：直接运行，按向导提示输入目标/端口/域名/并发等
./check.sh
...
# 4. 指定目标与多端口
./check.sh -t "AS206300 AS13335" -p "443,13720"

# 5. 按 CIDR 网段扫描并启用第三阶段域名校验
./check.sh -t 16.162.0.0/16 -p 443 -d example.com

# 6. 扫描后自动校验节点信息
./check.sh -t targets.txt -p 443 -v
```
